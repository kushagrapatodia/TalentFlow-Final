...
export function LoginPage() {
  ...
  useEffect(() => {
    if (role) navigate(role === 'hr' ? '/dashboard' : '/candidate/dashboard');
  }, [role, navigate]);
  ...
  return (
    <div className="container" style={{ paddingTop: 40, maxWidth: 520 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 16 }}>
        ...
        }} style={{ padding: 14, fontSize: 16, background: suggested === 'candidate' ? '#7c3aed' : undefined }}>
        ...
        <button className="btn" onClick={() => { 
          ...
          login('hr'); 
          navigate('/dashboard'); 
        }} style={{ padding: 14, fontSize: 16, background: suggested === 'hr' ? '#7c3aed' : undefined }}>
          Continue as HR
        </button>
      </div>
    </div>
