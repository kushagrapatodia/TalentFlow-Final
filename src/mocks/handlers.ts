...
export const handlers = [
  ...
  http.get('/candidates/:id/timeline', async ({ params }) => {
    await delay(randomDelay());
    const candidateId = String(params.id);
    let items = await db.timelines.where('candidateId').equals(candidateId).toArray();
  ...
  http.get('/candidates/:id/notes', async ({ params }) => {
    ...
    // Mock notes data - in a real app this would come from the database
    const mockNotes = [
      {
        id: crypto.randomUUID(),
        content: 'Great interview performance. @John Doe please schedule the technical round.',
        author: 'Sarah Wilson',
      ...
      {
        id: crypto.randomUUID(),
        content: 'Candidate has strong React experience. @Mike Johnson can you review their portfolio?',
        author: 'Jane Smith',
    ...
    return HttpResponse.json({ items: mockNotes });
  }),
  ...
  http.patch('/jobs/:id/reorder', async ({ params, request }) => {
    ...
    const id = String(params.id);
  const { fromOrder: _fromOrder, toOrder } = (await request.json()) as { fromOrder: number; toOrder: number };
